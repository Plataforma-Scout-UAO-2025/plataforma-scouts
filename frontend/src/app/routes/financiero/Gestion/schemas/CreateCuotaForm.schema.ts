...
export const createCuotaFormSchema = (isEditMode = false) =>
  z
    .object({
    ...
    .refine(
      ...
      {
        message:
          "Para periodicidad trimestral, el período debe ser de al menos 90 días",
        path: ["end_date"],
      }
    )
    .refine(
      ...
      {
        message:
          "Para periodicidad anual, el período debe ser de al menos 365 días",
        path: ["end_date"],
      }
    )
    .refine(
      ...
      {
        message:
          "La fecha de fin es requerida para periodicidades que no sean única",
        path: ["end_date"],
      }
    )
    .refine(
      (data) => {
        ...
        if (
          ...
          return (
            data.associated_to !== null &&
            data.associated_to.id !== "" &&
            data.associated_to.name !== ""
          );
        }
        return true;
      },
    ...
    .refine(
      (data) => {
        // Validar periodicidad anual: mínimo 365 días (solo si end_date existe)
        if (data.periodicity === "MONTH" && data.start_date && data.end_date) {
          const daysDifference = differenceInDays(
            data.end_date,
            data.start_date
          );
      ...
      {
        message: "Para periodicidad mensual, el período debe ser de 30 días",
        path: ["end_date"],
      }
    );

export type CreateCuotaFormValues = z.infer<
  ReturnType<typeof createCuotaFormSchema>
>;
