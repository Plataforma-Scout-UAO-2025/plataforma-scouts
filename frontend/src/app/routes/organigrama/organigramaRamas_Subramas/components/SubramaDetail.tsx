...
export default function SubramaDetail() {
  ...
  const [imageRefreshToken, setImageRefreshToken] = useState<number>(
    Date.now()
  );
  ...
  const subgroupId = subrama
    ? (((subrama as unknown as Record<string, unknown>)["subgroupId"] ??
        (subrama as unknown as Record<string, unknown>)["id"]) as number)
    : undefined;

  // Use Redux hook for members
  const {
    members: reduxMembers,
    loading: membersLoading,
    error: membersError,
    fetchMembers,
  } = useSubgroupMembers(subgroupId);
      ...
      try {
        ...
        if (uploadControllerRef.current) {
          try {
            ...
            console.warn("Could not abort previous upload controller", e);
          }
        }
        ...
        const sectionId =
          subrama.section_id ?? subrama.ramaId ?? subrama.branchId ?? "";
        ...
        try {
          ...
          console.debug("cleanup error", errCleanup);
        }
  ...
  const handleReplaceFoto = async (file: File) => {
    ...
    if (fotoTipo === "principal") {
      ...
      try {
        ...
        console.error(
          " [SubramaDetail] Error reemplazando imagen principal desde modal:",
          error
        );
        const msg =
          error instanceof Error
            ...
            ? ((error as unknown as Record<string, unknown>)[
                "message"
              ] as string) ?? String(error)
            : String(error);
        if (msg === "UploadCanceled" || msg === "canceled") {
          console.debug(
            "[SubramaDetail] modal upload canceled catch: previous=",
            previousImagenPrincipalRef.current,
            "imagenPrincipal=",
            imagenPrincipal
          );
          toast("Subida cancelada");
  ...
  const handleCancelUpload = () => {
    if (uploadControllerRef.current) {
      ...
      setCurrentUploadingFile(null);
      console.debug(
        "[SubramaDetail] handleCancelUpload: previousImagenPrincipalRef=",
        previousImagenPrincipalRef.current,
        "current imagenPrincipal=",
        imagenPrincipal
      );
  ...
  const fetchSubrama = useCallback(async () => {
    try {
      ...
      if (!hasTenantContext) {
        console.warn(
          "[SubramaDetail] Tenant o grupo no disponibles aún; omitiendo fetch"
        );
        return;
      }
      ...
      const ramas = await organigramaService.getRamasWithSubramas(
        resolvedTenantId,
        resolvedGroupSlug
      );
      ...
      if (subramaEncontrada) {
        ...
        console.log(
          ...
          subramaEncontrada.name ??
            subramaEncontrada.nombre ??
            subramaEncontrada.subgroupName
        );
        console.log(
        ...
        const backendMainImage = (
          subramaEncontrada as { mainImageUrl?: string }
        ).mainImageUrl;
        const resolvedMainImage =
          backendMainImage ?? subramaEncontrada.imagenPrincipal ?? "";

        if (resolvedMainImage) {
          if (
            !resolvedMainImage.startsWith("data:") &&
            resolvedMainImage.includes("http")
          ) {
            console.log(
          ...
          console.log(
            " [SubramaDetail] No hay imagen principal para subrama:",
            subramaEncontrada.name ??
              subramaEncontrada.nombre ??
              subramaEncontrada.subgroupName
          );
        ...
        const rawGallery = (
          subramaEncontrada as unknown as Record<string, unknown>
        )["gallery"] as unknown[] | undefined;
        const galleryUrls =
          Array.isArray(rawGallery) && rawGallery.length > 0
            ? (rawGallery as Array<Record<string, unknown>>)
                .map((g) => String(g.url))
                .filter(Boolean)
            : subramaEncontrada.galleryObjectIds ??
              subramaEncontrada.subgroupGalleryObjectIds ??
              [];

        if (galleryUrls && galleryUrls.length > 0) {
          console.log(
            ` [SubramaDetail] Cargadas ${galleryUrls.length} imágenes de galería desde backend`
          );
          console.log(" [SubramaDetail] URLs de galería:", galleryUrls);
        } else {
          console.log(
            " [SubramaDetail] No hay imágenes en la galería de la subrama"
          );
        }
      } else {
  ...
  const { uploadSubgroupPhotoPrincipal, deleteSubgroupPhotoPrincipal } =
    useOrganigramaActions({
      tenantId: resolvedTenantId,
      groupSlug: resolvedGroupSlug,
  ...
  useEffect(() => {
    if (subgroupId) {
      fetchMembers(subgroupId).catch((err) => {
        console.error(
          "[SubramaDetail] Error cargando miembros por subgrupo",
          err
        );
      });
    }
  ...
  if (isFetching || loading || !hasTenantContext || !subrama) {
    return (
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div>
            <Skeleton className="h-10 w-20" /> {/* Botón Anterior */}
          </div>
        </div>
        ...
        <Card className="p-4 space-y-4 bg-card text-card-foreground border border-border">
          ...
          <Skeleton className="h-[450px] w-full rounded-lg" /> {/* Imagen */}
          <div className="pt-4">
          ...
          <div className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <Skeleton className="h-4 w-40" /> {/* Título jefes */}
                <Skeleton className="h-4 w-16" /> {/* Conteo */}
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <Skeleton className="h-4 w-48" /> {/* Título scouts */}
                <Skeleton className="h-4 w-20" /> {/* Conteo */}
              </div>
              <div className="flex flex-wrap gap-2">
    ...
    <div className="space-y-6">
          ...
          {imagenPrincipal ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-[rgba(234,241,246,0.9)]">
              <div className="text-center">
                <span className="text-[72px] leading-none text-muted-foreground font-semibold">
                  Sin imagen
                </span>
              </div>
            </div>
      ...
      <Card className="p-4 space-y-3 bg-card text-card-foreground border border-border">
        ...
        {membersLoading ? (
          ...
          (() => {
            // Filtrar miembros activos y aprobados
            const activeMembers = reduxMembers.filter(
              isMemberActiveAndApproved
            );
              ...
              return (
                <div className="text-center py-4">
                  <p className="text-muted-foreground">
                    No hay integrantes activos en esta subrama.
                  </p>
                </div>
              );
            }
            ...
            const jefes = activeMembers.filter(isMemberScouter);
            const scouts = activeMembers.filter((m) => !isMemberScouter(m));
              ...
              <div className="space-y-4">
                ...
                {jefes.length > 0 && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {jefes.map((member, idx) => {
                        const memberWithRecord = member as typeof member &
                          Record<string, unknown>;
                        const display = getMemberFullName(memberWithRecord);
                        const memberId = getMemberId(memberWithRecord);
                        return (
                          <Badge
          ...
          <div className="text-center py-4">
            <p className="text-muted-foreground">
              No hay integrantes registrados en esta subrama.
            </p>
          </div>
        )}
      </Card>
