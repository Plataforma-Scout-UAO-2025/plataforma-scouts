...
export const mapBackendRamaToFrontend = (backendRama: BackendRama): Rama => {
  ...
  console.log(
    "🔄 [Mapper] Input del backend completo:",
    JSON.stringify(backendRama, null, 2)
  );
  ...
  const mappedRama = {
    ...
    fechaCreacion: backendRama.createdAt
      ? backendRama.createdAt.split("T")[0]
  ...
  console.log("🔄 [Mapper] Rama mapeada final:", {
    ...
    frontend: {
      nombre: mappedRama.nombre,
      id: mappedRama.id,
      icono: mappedRama.icono,
      imagenPrincipal: mappedRama.imagenPrincipal,
    },
  });
...
export const mapBackendSubramaToFrontend = (
  backendSubrama: BackendSubrama
): Subrama => {
  // 🔍 LOG DETALLADO: Ver qué campos exactos recibimos del backend para subramas
  console.log(
    "🔄 [Mapper] Input del backend para SUBRAMA:",
    JSON.stringify(backendSubrama, null, 2)
  );

  // Intentar extraer el ID canonical que provee el backend desde varios nombres posibles
  const rawId =
    backendSubrama.subgroup_id ??
    backendSubrama.subgroupId ??
    backendSubrama.id ??
    backendSubrama.ID ??
    backendSubrama.subgroupIdLegacy;
  ...
  const generateConsistentId = () => {
    ...
    const uniqueString = `${backendSubrama.subgroupName || ""}-${
      backendSubrama.section_id || ""
    }-${backendSubrama.subgroupDescription || ""}`;
  ...
  const mappedSubrama = {
    ...
    subgroupDescription:
      backendSubrama.subgroupDescription ||
      backendSubrama.subgroup_description ||
      backendSubrama.description,
    ...
    estado:
      backendSubrama.isActive === false || backendSubrama.status === "inactive"
        ? ("inactiva" as const)
        : ("activa" as const),
    fechaCreacion:
      backendSubrama.createdAt || new Date().toISOString().split("T")[0],
    numeroMiembros: backendSubrama.memberCount || backendSubrama.members || 0,
  };

  console.log("🔄 [Mapper] Subrama mapeada final:", {
    backend: {
      name: backendSubrama.subgroupName,
      subgroupId: backendSubrama.subgroup_id || backendSubrama.subgroupId,
    },
    frontend: {
