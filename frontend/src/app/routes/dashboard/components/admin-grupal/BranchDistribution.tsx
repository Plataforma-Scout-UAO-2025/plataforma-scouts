...
interface BranchDistributionProps {
  branchMembers: [string, number][];
}

const BranchDistribution = ({ branchMembers }: BranchDistributionProps) => {
  const total = branchMembers.reduce((sum, [, count]) => sum + count, 0);
  return (
    <div className="border rounded-xl shadow-sm p-6">
      ...
      <div className="space-y-4">
        {branchMembers.length === 0 ? (
          ...
          branchMembers.map(([rama, cantidad]) => {
