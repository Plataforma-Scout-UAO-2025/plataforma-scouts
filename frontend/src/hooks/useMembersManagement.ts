...
export const useMembersManagement = ({ itemsPerPage = 10 }: UseMembersManagementProps = {}) => {
  ...
  useEffect(() => {
    ...
    const filtered = members.filter((member: Member) => {
      ...
      const memberTenantId = (memberRec["tenant_id"] ?? memberRec["tenantId"]) as string | undefined;
      const matchesTenant = memberTenantId === tenantId;
      const firstName = (memberRec["first_name"] ?? memberRec["firstName"] ?? "") as string;
      const lastName = (memberRec["last_name"] ?? memberRec["lastName"] ?? "") as string;
      ...
      const isActiveField = (memberRec["is_active"] ?? memberRec["isActive"]) as boolean | undefined;
      const matchesStatus =
        statusFilter === "" ||
        mappingStatus(isActiveField).toLowerCase().includes(statusFilter.toLowerCase());
      ...
      const matchesBranch =
        ...
        (Array.isArray(member?.subgroup?.section)
          ...
          : member?.subgroup?.section?.name?.toLowerCase().includes(branchFilter.toLowerCase()));
    ...
    return filtered.map((member: Member): Member => {
      const normalized: Record<string, unknown> = { ...member } as Record<string, unknown>;

  const memberRec = member as unknown as Record<string, unknown>;
  const memberId = (memberRec["member_id"] ?? memberRec["memberId"]) as number | undefined;
  const firstName = (memberRec["first_name"] ?? memberRec["firstName"] ?? "") as string;
  const lastName = (memberRec["last_name"] ?? memberRec["lastName"] ?? "") as string;
  const createdAt = (memberRec["created_at"] ?? memberRec["createdAt"] ?? null) as string | null;
  const address = (memberRec["address"] ?? memberRec["address"] ?? "") as string;
  const isActive = (memberRec["is_active"] ?? memberRec["isActive"]) as boolean | undefined;
  ...
  const extractSectionsFromMember = (member: Member): string[] => {
      ...
      try {
          ...
          if (Array.isArray(subgroup)) {
            for (const sg of subgroup) {
              ...
              if (Array.isArray(sec)) {
                for (const s of sec) if (s && (s as Record<string, unknown>).name) names.push(String((s as Record<string, unknown>).name));
              } else if (sec && (sec as Record<string, unknown>).name) names.push(String((sec as Record<string, unknown>).name));
            }
          } else {
        ...
        if (Array.isArray(branchArr)) {
          for (const b of branchArr) if (b && (b as Record<string, unknown>).name) names.push(String((b as Record<string, unknown>).name));
        }
  
        // direct fields
        if (rec["sectionName"] && typeof rec["sectionName"] === "string") names.push(String(rec["sectionName"]));
        if (rec["section_name"] && typeof rec["section_name"] === "string") names.push(String(rec["section_name"]));
        if (rec["section"] && typeof rec["section"] === "object" && (rec["section"] as Record<string, unknown>).name) names.push(String((rec["section"] as Record<string, unknown>).name));
        ...
        if (mrec.subgroup && (mrec.subgroup as Record<string, unknown>)?.section && ((mrec.subgroup as Record<string, unknown>).section as Record<string, unknown>).name) {
          names.push(String(((mrec.subgroup as Record<string, unknown>).section as Record<string, unknown>).name));
        }
      } catch (err) {
  ...
  return {
    ...
    totalMembers:
      members
        ? members.filter((m) => {
            const t = (m["tenant_id"] ?? m["tenantId"]) as string | undefined;
            return t === tenantId;
          }).length
        : 0,    
    // Paginación
