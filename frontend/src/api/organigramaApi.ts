...
const mapBackendSectionToSection = (
  backendSection: Record<string, unknown>
): Section => {
  const section = backendSection as unknown as Section;
  return {
    ...section,
    id:
      (backendSection.sectionId as number) ||
      (backendSection.section_id as number),
    sectionId:
      (backendSection.sectionId as number | string) ||
      (backendSection.section_id as number | string),
    photoPrincipal:
      (backendSection.photoPrincipalId as string | null) ||
      (backendSection.photoPrincipal as string | null),
  };
};

const mapBackendSubgroupToSubgroup = (
  backendSubgroup: Record<string, unknown>
): Subgroup => {
  const subgroup = backendSubgroup as unknown as Subgroup;
  return {
    ...subgroup,
    id:
      (backendSubgroup.subgroupId as number) ||
      (backendSubgroup.subgroup_id as number),
    subgroupId:
      (backendSubgroup.subgroupId as number) ||
      (backendSubgroup.subgroup_id as number),
  };
...
export const getGroupsByTenant = async (
  tenantId: string,
  signal?: AbortSignal
) => {
  const config = signal ? { signal } : undefined;
  const response = await api.get<GroupResponseDTO[]>(
...
export const getGroupBySlug = async (tenantId: string, groupSlug: string) => {
  const response = await api.get<GroupResponseDTO>(
    `/tenants/${tenantId}/groups/${groupSlug}`
  );
  return response.data;
};
...
export const getSections = async (tenantId: string, groupSlug: string) => {
  const response = await api.get(
    `/tenants/${tenantId}/groups/${groupSlug}/sections`
  );
...
export const getSectionWithSubgroups = async (
  ...
  const config = signal ? { signal } : undefined;
  const response = await api.get(
    `/tenants/${tenantId}/groups/${groupSlug}/sections/${sectionId}/with-subgroups`,
    config
  );
  return {
    section: mapBackendSectionToSection(response.data.section || response.data),
    subgroups: (response.data.subgroups || []).map(
      mapBackendSubgroupToSubgroup
    ),
  };
};
...
export const createSection = async (
  payload: Omit<
    Section,
    "id" | "sectionId" | "groupId" | "createdAt" | "updatedAt"
  >,
  tenantId: string,
  groupSlug: string
) => {
...
export const getSubgroups = async (
  ...
  const response = await api.get(
    `/tenants/${tenantId}/groups/${groupSlug}/sections/${sectionId}/subgroups`
  );
...
export const getSubgroup = async (
  ...
  const response = await api.get(
    `/tenants/${tenantId}/groups/${groupSlug}/sections/${sectionId}/subgroups/${subgroupId}`
  );
...
export const uploadToGallery = async (
  ...
  const response = await api.post(
    `/tenants/${tenantId}/groups/${groupSlug}/sections/${sectionId}/gallery`,
    file,
...
export const setIcon = async (
  sectionId: string | number,
  payload: IconSetPayload,
  tenantId: string,
  groupSlug: string
) => {
  const response = await api.patch(
    `/tenants/${tenantId}/groups/${groupSlug}/sections/${sectionId}/icon`,
    payload
  );
  return response.data;
};
...
export const setPhotoPrincipal = async (
  sectionId: string | number,
  payload: Record<string, unknown>,
  tenantId: string,
  groupSlug: string
  ...
  const response = await api.patch(
    `/tenants/${tenantId}/groups/${groupSlug}/sections/${sectionId}/photo-principal`,
    payload
  );
  return response.data;
};
...
export const deletePhotoPrincipal = async (
  sectionId: string | number,
  tenantId: string,
  groupSlug: string
) => {
...
export const uploadSubgroupPhoto = async (
  sectionId: string | number,
  subgroupId: string | number,
  file: FormData,
  tenantId: string,
  ...
  const response = await api.post(
    `/tenants/${tenantId}/groups/${groupSlug}/sections/${sectionId}/subgroups/${subgroupId}/gallery`,
    file,
    { headers: { "Content-Type": "multipart/form-data" } }
  );
  return response.data;
...
export const patchSubgroupGallery = async (
  sectionId: string | number,
  subgroupId: string | number,
  payload: Record<string, unknown>,
  tenantId: string,
...
export const setSubgroupPhotoPrincipal = async (
  sectionId: string | number,
  subgroupId: string | number,
  payload: Record<string, unknown>,
  tenantId: string,
  ...
  const response = await api.patch(
    `/tenants/${tenantId}/groups/${groupSlug}/sections/${sectionId}/subgroups/${subgroupId}/photo-principal`,
    payload
  );
  return response.data;
};
...
export const deleteSubgroupPhotoPrincipal = async (
  sectionId: string | number,
  subgroupId: string | number,
  tenantId: string,
  groupSlug: string
...
export const deleteGalleryImageById = async (
  ...
  const endpoint = `/tenants/${tenantId}/groups/${groupSlug}/sections/${sectionId}/gallery/${objectId}?deleteFromStorage=${
    deleteFromStorage ? "true" : "false"
  }`;
